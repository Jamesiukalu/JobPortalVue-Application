            ...
            <div class="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
                <form @submit.prevent="onUpdateJob">
                    ...
                    <div class="mb-4">
                        ...
                        <select v-model="job.type" id="type" name="type" class="border rounded w-full py-2 px-3" required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea v-model="job.description" id="description" name="description"
                    ...
                    <div class="mb-4">
                        ...
                        <select v-model="job.salary" id="salary" name="salary" class="border rounded w-full py-2 px-3"
                            required>
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="job.company.contactEmail" type="email" id="contact_email" name="contact_email"
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="job.company.contactPhone" type="tel" id="contact_phone" name="contact_phone"
                            class="border rounded w-full py-2 px-3" placeholder="Optional phone for applicants" />
                    </div>

                    <div class="flex">
                        <button @click="onReturn"
                            class="bg-gray-600 hover:bg-gray-800 text-white text-center font-bold py-2 me-3 px-4 rounded-full w-full focus:outline-none focus:shadow-outline"
                            type="button">
                            Back to Jobs
                        </button>
                        <button
        ...
        async onUpdateJob() {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    this.toast.error('Job Was not Updated');
                    throw new Error(`Failed to update job: ${res.status}`);
                } else {
                    const updatedJob = await res.json();
                    this.toast.success('Job Updated Successfully');
                    this.router.push({ name: 'JobDetails', params: { id: updatedJob.id } });
                }
            } catch (error) {
                console.error('Error updating job:', error);
                this.toast.error('An error occurred while updating the job');
            } finally {
                this.loading = false;
            }
        },

            ...
            <div class="bg-white px-6 py-8 mb-4 shadow-md rounded-md border m-4 md:m-0">
                <form @submit="onSubmit">
                    ...
                    <div class="mb-4">
                        ...
                        <select v-model="type" id="type" name="type" class="border rounded w-full py-2 px-3" required>
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="title" type="text" id="name" name="name"
                            class="border rounded w-full py-2 px-3 mb-2" placeholder="Senior Vue Developer" required />
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select v-model="salary" id="salary" name="salary" class="border rounded w-full py-2 px-3"
                            required>
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="company.name" type="text" id="company" name="company"
                    ...
                    <div class="mb-4">
                        <label for="company_description" class="block text-gray-700 font-bold mb-2">Company
                            Description</label>
                        <textarea v-model="company.description" id="company_description" name="company_description"
                            class="border rounded w-full py-2 px-3" rows="4"
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="company.contactEmail" type="email" id="contact_email" name="contact_email"
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="company.contactPhone" type="tel" id="contact_phone" name="contact_phone"
...
export default {
    ...
    setup() {
        ...
        const toast = useToast(); // Corrected declaration
        return { router, toast };
    },
    methods: {
        onSubmit(e) {
        ...
        async addJob(job) {   
            ...
            try {
                const res = await fetch('/api/jobs', {
                    ...
                    body: JSON.stringify(job), // Assuming newJob is defined
                });

                if (!res.ok) {
                    // Handle non-OK responses (e.g., 4xx or 5xx)
                    const errorText = await res.text(); // Get error text from the response
                    console.error(`Failed to add job: ${res.status}`, errorText); 
                    this.toast.error('Job Was not Added');
                    throw new Error(`Failed to add job: ${res.status}`);
                } else {
                    const newJob = await res.json();
                    this.toast.success('Job Added Successfully');
                    this.router.push({ name: 'JobDetails', params: { id: newJob.id } });
                }
            } catch (error) {
                // Handle any other errors (e.g., network issues)
                console.error('Error adding job:', error);
                this.toast.error('An error occurred while adding the job');
            } finally {
                this.loading = false;
            }
